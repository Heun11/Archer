...
void PARTICLE_Render_Smoke(PARTICLE_Particles* ps, int life_time, int ts)
{
    for(int i=0;i<ps->size;i++){
        ps->ps[i].y -= ts*elapsed*((float)TOOLS_Random_Number(1,50)/10.f);
        ps->ps[i].x += ts*TOOLS_Random_Number(-3, 3)*elapsed;
        ...
        SDL_SetRenderDrawColor(rend, 252, 3, 240, (int)(255-255*ps->ps[i].a));
