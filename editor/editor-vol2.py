		...
		if("ts" in content):
			...
			for i in range(len(content["imgs"])):
				...
				for j in content["imgs"][i]:
					...
					num_to_img[i][int(j)] = pygame.transform.scale(load_from_ts(ts, int(content["imgs"][i][j][0]), int(content["imgs"][i][j][1]), content["ts_s"], content["ts_s"]), (size_of_square, size_of_square))
		else:
